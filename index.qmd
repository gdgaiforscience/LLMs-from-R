...
-   Install the [`ellmer`](https://ellmer.tidyverse.org/),[`ragnar`](https://ragnar.tidyverse.org), and [`chattr`](https://mlverse.github.io/chattr/) package from CRAN as below. *ragnar* may take up to about 1 hour to install. These are working on RStudio/R 4.5.3 on Mac. Your device and OS may need system level packages. 
...
> Note: I needed to also add additional packages to my setup inclusing: `install.packages(c("coro","withr","curl","vctrs","reticulate","duckdb","blob","commonmark","dplyr","promises","shiny"), lib="./Rlibs")` - you may have to install a few on your local machine.
