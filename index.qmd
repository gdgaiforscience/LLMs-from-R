...
-   Install the [`ellmer`](https://ellmer.tidyverse.org/),[`ragnar`](https://ragnar.tidyverse.org), and [`chattr`](https://mlverse.github.io/chattr/) package from CRAN as below. *ragnar* may take up to about 1 hour to install. These have been tested as working on RStudio 2026.01.2 and R 4.5.3 on Mac Silicon, plus R 4.4.3 and above on Windows.
...
> Note: I needed additional packages for my setup including: `install.packages(c("coro","withr","curl","vctrs","reticulate","duckdb","blob","commonmark","dplyr","promises","shiny","gargle"), lib="./Rlibs")` - you may have to install a few on your local machine.

-   Set up API keys from [https://aistudio.google.com/](https://aistudio.google.com/) or equivalent.

> Note: Be aware of the [Free Tier Rate/Usage limits](https://aistudio.google.com/rate-limit). The `gemini-2.5-flash-lite`, `gemini-3.1-flash-lite-preview`, `gemini-2.5-flash`, and `gemini-3-flash-preview` models have the most free-tier. When using the paid-tier you must use the  [`chat_google_vertex`](https://ellmer.tidyverse.org/reference/chat_google_gemini.html) endpoints to take advantage of cloud credits.
...
chattr_app()
```
Or send a quick prompt directly. Note where you call it from, will be where the repsonse is returned!
```{r}
chattr("Make me a ggplot example")
```

Now add more context to your calls!
Start by defining a function for chattr to handle:
```{r}
get_results_columns <- function() {
  if (exists("results_data")) {
    cols <- paste(colnames(sales_data), collapse = ", ")
    return(paste("The results_data data frame has the following columns:", cols))
  }
  return("results_data does not exist yet.")
}
```

Then inject it into the prompt defaults
```{r}
chattr_defaults(prompt = "{ get_results_columns() }")
```

Create a small mock data set
```{r}
results_data <- data.frame(Date = "2026-04-20", Amount = 150, ID = 3)
```
Now ask a question! The LLM will already know the columns are Date, Amount, and ID.
```{r}
chattr("Write code to group by id and sum the amounts.")
```

## Notes for use in Google Colab
`ellmer` works as normal. 

`ragnar` takes a long time to install but otherwise works as expected.

`chattr` can't connect to the interactive shiny app, but you can still use command line text-based syntax.
