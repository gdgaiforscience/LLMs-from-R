...
[![ ](./aiforsci.png){style="max-height: 200px;"}](https://gdg.community.dev/gdg-ai-for-science-australia/) [![ ](./rladies.png){style="max-height: 200px;"}](https://www.meetup.com/rladies-sydney/)

This workshop is part of the [GDG AI for Science](https://gdg.community.dev/gdg-ai-for-science-australia/) workshop series collaborating with the [R-Ladies](https://rladiessydney.org/). Join the communities for talks, events, collaborations and more.


## **Installation and Setup**

-   Install the [`ellmer`](https://ellmer.tidyverse.org/),[`ragnar`](https://ragnar.tidyverse.org), and [`chattr`](https://mlverse.github.io/chattr/) package from CRAN as below. *ragnar* may take up to about 1 hour to install. These are working on RStudio 2026.01.2 and R 4.5.3 on Mac Silicon. Works on R 4.4.3 and above on Windows as well.
...
> Note: I needed additional packages for my setup including: `install.packages(c("coro","withr","curl","vctrs","reticulate","duckdb","blob","commonmark","dplyr","promises","shiny"), lib="./Rlibs")` - you may have to install a few on your local machine.

-   Set up API keys from [https://aistudio.google.com/](https://aistudio.google.com/) or equivalent.
```{r}
...
-   **Prompts and context:** Understand the basics of what the LLM knows and what your environment can see.


### Your First Chat

These code snippets will guide you through the basics of the `ellmer` package. Always start by loading the ellmer package - your gateway to LLMs in R. And setting an API key.
...
# We'll use the gemini flash model as it's fast and cost-effective.
...
```
The LLM *should* provide you with the R code and an explanation. This is much faster than searching through documentation or web forums. But note, asking questions does not send your R-environment's context/variables/history/etc to the LLM.
...
# Use a "system prompt" to give the LLM a persona.
# This guides its responses to be more focused and relevant.
biostat_chat <- chat_google_gemini(
  model = "gemini-flash-latest", 
  system_prompt = "You are a helpful and experienced biostatistician.
  Your goal is to provide clear, practical advice on experimental design for biomedical researchers.
  Do not write R code unless explicitly asked."
)
...
translator_chat <- chat_google_gemini(model = "gemini-flash-latest")
...
abstract_text <- "The role of gene XYZ in cellular metabolism was investigated in the murine model. Using a cohort of 60 male C57BL/6J mice (30 wild-type, 30 knockout for gene XYZ), we measured serum glucose levels following a 6-hour fast. 
A two-sample t-test revealed significantly higher glucose levels in the knockout group (p < 0.001) compared to wild-type controls. These results strongly suggest that gene XYZ plays a critical role in maintaining glucose homeostasis."
...
```
Imagine running this in a loop over hundreds of abstracts - a huge time saver! Read more about [ellmer's structured data](https://ellmer.tidyverse.org/articles/structured-data.html) and [type specifications](https://ellmer.tidyverse.org/reference/type_boolean.html).
...
-   **Document Processing and Chunking:** Learn to process different document types (e.g., markdown, text files) and strategies for splitting them into manageable chunks.
...
Grab a few markdown files from [here](https://github.com/gdgaiforscience/LLMs-from-R/raw/refs/heads/main/lab_sops.zip). Unzip the three `*.md` files and point to the directory below. These are in markdown format so follow a nice natural context for chunking/tokenizing. If your documents are in different formats, you may have to use different chunking strategies.
...
store <- ragnar_store_create(
  ...
  overwrite = TRUE #Overwrite the current database if it exists
)
...
Here we read in all our (markdown) documents, we "chunk" them into some kind of contextual tokens (usually sections, paragraphs, sentences, etc), we then convert the chunks into a numerical representation based on how this "embedding model" links semantically similar tokens/chunks. See the docs for more info:
...
# Expected output for Q3: I'm sorry, but the information regarding the protocol for 
# performing a comet assay is not available in the lab protocols.
```
...
### Chattr in RStudio
Load the chattr package.
```{r}
library(chattr)
```
```{r}
# Create an ellmer chat connection,
chat <- ellmer::chat_google_gemini(
  model = "gemini-flash-latest"
  )
# then pass that chat object to chattr_use()
chattr_use(chat)
```

Now call the app!
```{r}
# Launch the Shiny gadget. Test a prompt in the pop-up that shows in Viewer.
chattr_app()
```
```{r}
# Or send a quick prompt directly
chattr("Make me a ggplot example")
```

### Chattr in Google Colab
This would work quite differently from the method used for RStudio.
Start with a new Google colab notebook and change the runtime type to R.
```{r}
install.packages("chattr")
install.packages("ellmer")
```
```{r}
# The notebook is currently using an R kernel.
# Set the Gemini API key in R for packages to use:
Sys.setenv(GEMINI_API_KEY = "xxxx")
```
```{r}
chat <- ellmer::chat_google_gemini(
...
```
chattr_use() sets the active model.
```{r}
chattr::chattr_use(chat)
```
```{r}
# chattr_test() verifies that the connection to Gemini is working properly
chattr::chattr_test()
```
```{r}
chattr::chattr("Make me a ggplot example")
```
```{r}
chattr::chattr("Customize the previous ggplot example with a different theme, like theme_dark() or theme_bw()")
```
Generate a ggplot visual using the generated chattr plot
```{r}
install.packages("tidyverse")
library(tidyverse)
# Generating the fuel efficiency scatter plot with theme_dark()
# Plot the relationship between city and highway mileage
ggplot(data = mpg, mapping = aes(x = cty, y = hwy, color = class)) +
  geom_point(size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, formula = y ~ x) +
  labs(
    title = "Fuel Efficiency: City vs Highway Mileage",
    subtitle = "Visualized using theme_dark()",
...
```
```{r}
# Save the generated ggplot to a PNG file
# Adjust the width, height, and dpi (resolution) as needed
ggsave("fuel_efficiency_plot.png", width = 8, height = 6, dpi = 300)

print("Plot saved to 'fuel_efficiency_plot.png'!")
```
```{r}
# Ask the LLM for more complex ggplot2 customizations
chattr::chattr("Update the previous ggplot fuel efficiency example with more complex customizations: add faceting by the 'drv' (drive train) variable, use a custom color palette like scale_color_viridis_d(), and modify the theme to adjust the base text size and legend position.")
```
```{r}
# library(shiny)
# chattr::chattr_app()
```
Because Colab runs in the cloud, chattr_app() will start a local web server and print a local URL (e.g., http://127.0.0.1:xxxx). You won't be able to open this URL directly in your browser because your computer is separate from the Colab virtual machine.

To interact with chattr effectively inside Colab, it is usually best to stick to the text-based chattr::chattr("your prompt") command like we did previously!

Tips for recreating above modules on Colab:
- Skip the `./Rlibs` custom library path — use plain `install.packages()`.
- Download the SOPs programmatically instead of manually:
```{r}
  download.file("https://github.com/gdgaiforscience/LLMs-from-R/raw/refs/heads/main/lab_sops.zip", "lab_sops.zip")
  unzip("lab_sops.zip")
```
...
Dive deeper into LLM-support in R with these packages:
 
 - [ollamar](https://hauselin.github.io/ollama-r/) for local LLMs.
 - [shinychat](https://posit-dev.github.io/shinychat/r/index.html) for a Shiny-based chat application.
 - [vitals](https://vitals.tidyverse.org/) for eval benchmarks and grounding of your LLMs.
 - [mcptools](https://posit-dev.github.io/mcptools/) to interact with Model Context Protocol servers.
 - [gander](https://simonpcouch.github.io/gander/) RStudio completions like Copilot.
 - [mall](https://mlverse.github.io/mall/) for LLM/NLP predictions on your data.
 - [chores](https://simonpcouch.github.io/chores/) ergonomic LLM assistants in RStudio.
